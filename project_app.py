...
st.set_page_config(page_title="Churn Prediction App", page_icon="🧊", layout='centered', initial_sidebar_state='expanded')
...
st.sidebar.header("Employee churn analysis aims to predict who will leave the company. ")
...
with col1:
    result1=st.button("Click here for the model1")
 
with col2:
    result2=st.button("Click here for the model2")
...
if result1:
        ...
        #satisfaction_level
        satisfaction= st.slider("YOUR COMPANY SATISFACTION LEVEL",0.,max(df["satisfaction_level"]) ,0.50)
        #salary
        salary=df["salary"].unique().tolist()
        salary=st.selectbox("YOUR SALARY LEVEL", salary)
        #last_evaluation
        last_evaluation= st.slider("YOUR FINAL EVALUATION",0.,max(df["last_evaluation"]) ,0.50)
        ...
        #number_project
        number_project= st.slider("NUMBER OF PROJECTS YOU WORKED",0,max(df["number_project"]) ,3)
        #Work accident
        ...
        df_model=pd.DataFrame.from_dict([my_dict])
        #st.table(df_model)
        import pickle

        filename ="HR_Dataset_model_xgb.pkl"
        model =pickle.load(open('HR_Dataset_model_xgb.pkl', 'rb')) 
        myButton = st.button("Predict Churn")
        button_style = """
            <style>
            .stButton > button {
                color: lawngreen;
...
if result2:
        ...
        filename ="2HR_Dataset_model_xgb.pkl"
...
st.write("")
st.write("")
st.subheader("**_YOUR SELECTED_**;")
#st.write(f"DEPARTMENTS:{Departments}, COMPANY SATISFACTION LEVEL:{     satisfaction},SALARY LEVEL:{salary},")
#st.write(f"FINAL EVALUATION:{last_evaluation},   Promotions in the last 5 years  :{promotion}")       
#st.write(f"AVERAGE WORKING HOURS : {average_montly_hours},COMPANY WORKING YEAR :{company},")
#st.write(f"NUMBER OF PROJECTS YOU WORKED {number_project},WORK ACCIDENT {Work_accident}.")
